...
public:
	...
	Avatar(glm::vec3 avatarPosition , glm::vec3 avatarOrintation)
	{
		...
		Plane = std::unique_ptr<Model3D>(new Model3D());
		//Plane->LoadFromFile("data/models/Jeep/jeep1.3ds",false);
		Plane->LoadFromFile("data/Heinkel/HEINKEL.3DS", true);
		Plane->Initialize();

		CarMat = glm::translate(position) * glm::scale(0.5f,0.5f,0.2f);
	...
	void UpdateMatrix()
	{
		CarMat = glm::translate(position) * glm::scale(0.3f,0.3f,0.3f) ;
		CarMat *= glm::orientation(orintation,glm::vec3(0,0,1));
		//CarMat *= glm::rotate(-90.0f, glm::vec3(1,0,0));
	}
	void Draw( ShaderProgram shader)
	{
		Plane->Render(&shader, CarMat);
	}
